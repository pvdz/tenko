...
`````
throws: Lexer error!
    Regex: A double unicode quad escape that represents a surrogate pair in char class or group name is only valid with u-flag or v-flag; Encountered invalid unicode escape inside the group name of a `\k` escape, this is invalid with u-flag or v-flag; Found a `\k` that referenced `@{xd87e}@xyz` but no capturing group had this name
