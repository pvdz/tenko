...
`````
throws: Lexer error!
    Regex: Encountered invalid unicode escape inside the group name of a `\k` escape, this is invalid with u-flag or v-flag; Found a `\k` that referenced `@{xd87e}@xyz` but no capturing group had this name
