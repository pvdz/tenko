...
`````
throws: Lexer error!
    Regex: The `\p` escaped binary property name `ASCII` is not valid (does not appear in "table 54"); Regex contained syntax that is invalid with the u-flag but the u-flag was present
