...
`````
throws: Lexer error!
    Regex: Character class escapes `\d \D \s \S \w \W \p \P` not allowed in ranges with u; Encountered incorrect range (left>right, 125 > 122, 0x7d > 0x7a) when parsing as if without u-flag
