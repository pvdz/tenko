...
- Path: tests/testcases/using/for_await_expr_async_pass.md

> :: using
>
> ::> for await expr async pass
>
> `for (await x;;)` in async function should pass @{x2014}@ `await x` is a valid await expression in the for-init
...
tokens (17x):
       ...
       PUNC_CURLY_OPEN ID_for PUNC_PAREN_OPEN ID_await IDENT PUNC_SEMI
