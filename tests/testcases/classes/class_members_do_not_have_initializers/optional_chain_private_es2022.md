# Tenko parser test case

- Path: tests/testcases/classes/class_members_do_not_have_initializers/optional_chain_private_es2022.md

> :: classes : class members do not have initializers
>
> ::> optional chain private es2022
>
> this?.#x optional chaining with private member (requires optional chaining + class fields)

## PASS

## Input

`````js
class C { #x = 1; m() { return this?.#x; } }
`````

## Output

_Note: the whole output block is auto-generated. Manual changes will be overwritten!_

Below follow outputs in five parsing modes: sloppy, sloppy+annexb, strict script, module, module+annexb.

Note that the output parts are auto-generated by the test runner to reflect actual result.

### Sloppy mode

Parsed with script goal and as if the code did not start with strict mode header.

`````
ast: {
  type: 'Program',
  loc:{start:{line:1,column:0},end:{line:1,column:44},source:''},
  body: [
    {
      type: 'ClassDeclaration',
      loc:{start:{line:1,column:0},end:{line:1,column:44},source:''},
      id: {
        type: 'Identifier',
        loc:{start:{line:1,column:6},end:{line:1,column:7},source:''},
        name: 'C'
      },
      superClass: null,
      body: {
        type: 'ClassBody',
        loc:{start:{line:1,column:8},end:{line:1,column:44},source:''},
        body: [
          {
            type: 'PropertyDefinition',
            loc:{start:{line:1,column:10},end:{line:1,column:16},source:''},
            key: {
              type: 'PrivateIdentifier',
              name: 'x',
              loc:{start:{line:1,column:10},end:{line:1,column:12},source:''}
            },
            value: {
              type: 'Literal',
              loc:{start:{line:1,column:15},end:{line:1,column:16},source:''},
              value: 1,
              raw: '1'
            },
            computed: false,
            static: false
          },
          {
            type: 'MethodDefinition',
            loc:{start:{line:1,column:18},end:{line:1,column:42},source:''},
            key: {
              type: 'Identifier',
              loc:{start:{line:1,column:18},end:{line:1,column:19},source:''},
              name: 'm'
            },
            static: false,
            computed: false,
            kind: 'method',
            value: {
              type: 'FunctionExpression',
              loc:{start:{line:1,column:18},end:{line:1,column:42},source:''},
              generator: false,
              async: false,
              id: null,
              params: [],
              body: {
                type: 'BlockStatement',
                loc:{start:{line:1,column:22},end:{line:1,column:42},source:''},
                body: [
                  {
                    type: 'ReturnStatement',
                    loc:{start:{line:1,column:24},end:{line:1,column:40},source:''},
                    argument: {
                      type: 'ChainExpression',
                      loc:{start:{line:1,column:31},end:{line:1,column:39},source:''},
                      expression: {
                        type: 'MemberExpression',
                        loc:{start:{line:1,column:31},end:{line:1,column:39},source:''},
                        computed: false,
                        optional: true,
                        object: {
                          type: 'ThisExpression',
                          loc:{start:{line:1,column:31},end:{line:1,column:35},source:''}
                        },
                        property: {
                          type: 'PrivateIdentifier',
                          name: 'x',
                          loc:{start:{line:1,column:37},end:{line:1,column:39},source:''}
                        }
                      }
                    }
                  }
                ]
              }
            }
          }
        ]
      }
    }
  ]
}

tokens (19x):
       ID_class IDENT PUNC_CURLY_OPEN ID_PRIVATE_IDENT PUNC_EQ
       NUMBER_DEC PUNC_SEMI IDENT PUNC_PAREN_OPEN PUNC_PAREN_CLOSE
       PUNC_CURLY_OPEN ID_return ID_this QMARK_DOT ID_PRIVATE_IDENT
       PUNC_SEMI PUNC_CURLY_CLOSE PUNC_CURLY_CLOSE
`````

### Strict mode

Parsed with script goal but as if it was starting with `"use strict"` at the top.

_Output same as sloppy mode._

### Module goal

Parsed with the module goal.

_Output same as sloppy mode._

### Sloppy mode with AnnexB

Parsed with script goal with AnnexB rules enabled and as if the code did not start with strict mode header.

_Output same as sloppy mode._

### Module goal with AnnexB

Parsed with the module goal with AnnexB rules enabled.

_Output same as sloppy mode._

## AST Printer

Printer output different from input [sloppy][annexb:no]:

````js
class C{#x = 1;
m(){return (this?.#x);};}
````

Produces same AST
