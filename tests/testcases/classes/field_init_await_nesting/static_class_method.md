...
- Path: tests/testcases/classes/field_init_await_nesting/static_class_method.md
...
`````js
class A { static x = class { m(){ return await } } }
`````
...
tokens (19x):
       ID_class IDENT PUNC_CURLY_OPEN ID_static IDENT PUNC_EQ ID_class
...
╔══╦═════════════════
 1 ║ class A { static x = class { m(){ return await } } }
   ║                                                ^------- error
╚══╩═════════════════
