...
- Path: tests/testcases/classes/field_init_await_nesting/instance_function_name.md
...
`````js
class A { x = function g(){ return await } }
`````
...
tokens (16x):
       ID_class IDENT PUNC_CURLY_OPEN IDENT PUNC_EQ ID_function IDENT
       PUNC_PAREN_OPEN PUNC_PAREN_CLOSE PUNC_CURLY_OPEN ID_return
       ID_await ASI PUNC_CURLY_CLOSE PUNC_CURLY_CLOSE
`````
...
╔══╦═════════════════
 1 ║ class A { x = function g(){ return await } }
   ║                                          ^------- error
╚══╩═════════════════
