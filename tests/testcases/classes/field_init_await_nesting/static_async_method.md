...
- Path: tests/testcases/classes/field_init_await_nesting/static_async_method.md
...
`````js
class A { static x = ({async m(){ await 1 }}) }
`````
...
tokens (21x):
       ...
       PUNC_PAREN_CLOSE PUNC_CURLY_OPEN ID_await NUMBER_DEC ASI
       PUNC_CURLY_CLOSE PUNC_CURLY_CLOSE PUNC_PAREN_CLOSE
       PUNC_CURLY_CLOSE
`````
