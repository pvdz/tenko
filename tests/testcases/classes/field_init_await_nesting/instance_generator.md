...
- Path: tests/testcases/classes/field_init_await_nesting/instance_generator.md
...
`````js
class A { x = function*(){ return await } }
`````
...
tokens (16x):
       ID_class IDENT PUNC_CURLY_OPEN IDENT PUNC_EQ ID_function
...
╔══╦═════════════════
 1 ║ class A { x = function*(){ return await } }
   ║                                         ^------- error
╚══╩═════════════════
