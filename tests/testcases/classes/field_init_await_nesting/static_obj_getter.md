...
- Path: tests/testcases/classes/field_init_await_nesting/static_obj_getter.md
...
`````js
class A { static x = ({get m(){ return await }}) }
`````
...
tokens (21x):
       ...
       PUNC_PAREN_CLOSE PUNC_CURLY_OPEN ID_return ID_await ASI
...
╔══╦═════════════════
 1 ║ class A { static x = ({get m(){ return await }}) }
   ║                                              ^------- error
╚══╩═════════════════
