# Tenko parser test case

- Path: tests/testcases/classes/private_in/valid_private_in_chained_with_regular.md

> :: classes : private in
>
> ::> valid private in chained with regular
>
> Valid: #field in ShiftExpression followed by stronger/different op

## PASS

## Input

`````js
class C { #field; constructor() { #field in this == true; } }
`````

## Output

_Note: the whole output block is auto-generated. Manual changes will be overwritten!_

Below follow outputs in five parsing modes: sloppy, sloppy+annexb, strict script, module, module+annexb.

Note that the output parts are auto-generated by the test runner to reflect actual result.

### Sloppy mode

Parsed with script goal and as if the code did not start with strict mode header.

`````
ast: {
  type: 'Program',
  loc:{start:{line:1,column:0},end:{line:1,column:61},source:''},
  body: [
    {
      type: 'ClassDeclaration',
      loc:{start:{line:1,column:0},end:{line:1,column:61},source:''},
      id: {
        type: 'Identifier',
        loc:{start:{line:1,column:6},end:{line:1,column:7},source:''},
        name: 'C'
      },
      superClass: null,
      body: {
        type: 'ClassBody',
        loc:{start:{line:1,column:8},end:{line:1,column:61},source:''},
        body: [
          {
            type: 'PropertyDefinition',
            loc:{start:{line:1,column:10},end:{line:1,column:16},source:''},
            key: {
              type: 'PrivateIdentifier',
              name: 'field',
              loc:{start:{line:1,column:10},end:{line:1,column:16},source:''}
            },
            value: null,
            computed: false,
            static: false
          },
          {
            type: 'MethodDefinition',
            loc:{start:{line:1,column:18},end:{line:1,column:59},source:''},
            key: {
              type: 'Identifier',
              loc:{start:{line:1,column:18},end:{line:1,column:29},source:''},
              name: 'constructor'
            },
            static: false,
            computed: false,
            kind: 'constructor',
            value: {
              type: 'FunctionExpression',
              loc:{start:{line:1,column:18},end:{line:1,column:59},source:''},
              generator: false,
              async: false,
              id: null,
              params: [],
              body: {
                type: 'BlockStatement',
                loc:{start:{line:1,column:32},end:{line:1,column:59},source:''},
                body: [
                  {
                    type: 'ExpressionStatement',
                    loc:{start:{line:1,column:34},end:{line:1,column:57},source:''},
                    expression: {
                      type: 'BinaryExpression',
                      loc:{start:{line:1,column:34},end:{line:1,column:56},source:''},
                      left: {
                        type: 'BinaryExpression',
                        loc:{start:{line:1,column:34},end:{line:1,column:48},source:''},
                        left: {
                          type: 'PrivateIdentifier',
                          name: 'field',
                          loc:{start:{line:1,column:34},end:{line:1,column:40},source:''}
                        },
                        operator: 'in',
                        right: {
                          type: 'ThisExpression',
                          loc:{start:{line:1,column:44},end:{line:1,column:48},source:''}
                        }
                      },
                      operator: '==',
                      right: {
                        type: 'Literal',
                        loc:{start:{line:1,column:52},end:{line:1,column:56},source:''},
                        value: true,
                        raw: 'true'
                      }
                    }
                  }
                ]
              }
            }
          }
        ]
      }
    }
  ]
}

tokens (18x):
       ID_class IDENT PUNC_CURLY_OPEN ID_PRIVATE_IDENT PUNC_SEMI IDENT
       PUNC_PAREN_OPEN PUNC_PAREN_CLOSE PUNC_CURLY_OPEN
       ID_PRIVATE_IDENT ID_in ID_this PUNC_EQ_EQ ID_true PUNC_SEMI
       PUNC_CURLY_CLOSE PUNC_CURLY_CLOSE
`````

### Strict mode

Parsed with script goal but as if it was starting with `"use strict"` at the top.

_Output same as sloppy mode._

### Module goal

Parsed with the module goal.

_Output same as sloppy mode._

### Sloppy mode with AnnexB

Parsed with script goal with AnnexB rules enabled and as if the code did not start with strict mode header.

_Output same as sloppy mode._

### Module goal with AnnexB

Parsed with the module goal with AnnexB rules enabled.

_Output same as sloppy mode._

## AST Printer

Printer output different from input [sloppy][annexb:no]:

````js
class C{
#field;
constructor(){(#field in this) == true;};
}
````

Produces same AST
