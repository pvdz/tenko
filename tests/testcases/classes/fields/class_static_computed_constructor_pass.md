# Tenko parser test case

- Path: tests/testcases/classes/fields/class_static_computed_constructor_pass.md

> :: classes : fields
>
> ::> class static computed constructor pass
>
> Static field with computed key constructor is allowed (PropName is empty for computed).

## PASS

## Input

`````js
var C = class { static ['constructor']; };
`````

## Output

_Note: the whole output block is auto-generated. Manual changes will be overwritten!_

Below follow outputs in five parsing modes: sloppy, sloppy+annexb, strict script, module, module+annexb.

Note that the output parts are auto-generated by the test runner to reflect actual result.

### Sloppy mode

Parsed with script goal and as if the code did not start with strict mode header.

`````
ast: {
  type: 'Program',
  loc:{start:{line:1,column:0},end:{line:1,column:42},source:''},
  body: [
    {
      type: 'VariableDeclaration',
      loc:{start:{line:1,column:0},end:{line:1,column:42},source:''},
      kind: 'var',
      declarations: [
        {
          type: 'VariableDeclarator',
          loc:{start:{line:1,column:4},end:{line:1,column:41},source:''},
          id: {
            type: 'Identifier',
            loc:{start:{line:1,column:4},end:{line:1,column:5},source:''},
            name: 'C'
          },
          init: {
            type: 'ClassExpression',
            loc:{start:{line:1,column:8},end:{line:1,column:41},source:''},
            id: null,
            superClass: null,
            body: {
              type: 'ClassBody',
              loc:{start:{line:1,column:14},end:{line:1,column:41},source:''},
              body: [
                {
                  type: 'PropertyDefinition',
                  loc:{start:{line:1,column:16},end:{line:1,column:38},source:''},
                  key: {
                    type: 'Literal',
                    loc:{start:{line:1,column:24},end:{line:1,column:37},source:''},
                    value: 'constructor',
                    raw: "'constructor'"
                  },
                  value: undefined,
                  computed: true,
                  static: true
                }
              ]
            }
          }
        }
      ]
    }
  ]
}

tokens (13x):
       ID_var IDENT PUNC_EQ ID_class PUNC_CURLY_OPEN ID_static
       PUNC_BRACKET_OPEN STRING_SINGLE PUNC_BRACKET_CLOSE PUNC_SEMI
       PUNC_CURLY_CLOSE PUNC_SEMI
`````

### Strict mode

Parsed with script goal but as if it was starting with `"use strict"` at the top.

_Output same as sloppy mode._

### Module goal

Parsed with the module goal.

_Output same as sloppy mode._

### Sloppy mode with AnnexB

Parsed with script goal with AnnexB rules enabled and as if the code did not start with strict mode header.

_Output same as sloppy mode._

### Module goal with AnnexB

Parsed with the module goal with AnnexB rules enabled.

_Output same as sloppy mode._

## AST Printer

Printer output different from input [sloppy][annexb:no]:

````js
var C = class{static ['constructor'];};
````

Produces same AST
