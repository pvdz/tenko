# Tenko parser test case

- Path: tests/testcases/classes/static_block_then_member/sb_priv_in_then_gen.md

> :: classes : static block then member
>
> ::> sb priv in then gen
>
> static block private-in field with following generator

## PASS

## Input

`````js
class C{#x;static{}*m(){#x in this}}
`````

## Output

_Note: the whole output block is auto-generated. Manual changes will be overwritten!_

Below follow outputs in five parsing modes: sloppy, sloppy+annexb, strict script, module, module+annexb.

Note that the output parts are auto-generated by the test runner to reflect actual result.

### Sloppy mode

Parsed with script goal and as if the code did not start with strict mode header.

`````
ast: {
  type: 'Program',
  loc:{start:{line:1,column:0},end:{line:1,column:36},source:''},
  body: [
    {
      type: 'ClassDeclaration',
      loc:{start:{line:1,column:0},end:{line:1,column:36},source:''},
      id: {
        type: 'Identifier',
        loc:{start:{line:1,column:6},end:{line:1,column:7},source:''},
        name: 'C'
      },
      superClass: null,
      body: {
        type: 'ClassBody',
        loc:{start:{line:1,column:7},end:{line:1,column:36},source:''},
        body: [
          {
            type: 'PropertyDefinition',
            loc:{start:{line:1,column:8},end:{line:1,column:10},source:''},
            key: {
              type: 'PrivateIdentifier',
              name: 'x',
              loc:{start:{line:1,column:8},end:{line:1,column:10},source:''}
            },
            value: null,
            computed: false,
            static: false
          },
          {
            type: 'StaticBlock',
            loc:{start:{line:1,column:17},end:{line:1,column:19},source:''},
            body: []
          },
          {
            type: 'MethodDefinition',
            loc:{start:{line:1,column:19},end:{line:1,column:35},source:''},
            key: {
              type: 'Identifier',
              loc:{start:{line:1,column:20},end:{line:1,column:21},source:''},
              name: 'm'
            },
            static: false,
            computed: false,
            kind: 'method',
            value: {
              type: 'FunctionExpression',
              loc:{start:{line:1,column:19},end:{line:1,column:35},source:''},
              generator: true,
              async: false,
              id: null,
              params: [],
              body: {
                type: 'BlockStatement',
                loc:{start:{line:1,column:23},end:{line:1,column:35},source:''},
                body: [
                  {
                    type: 'ExpressionStatement',
                    loc:{start:{line:1,column:24},end:{line:1,column:34},source:''},
                    expression: {
                      type: 'BinaryExpression',
                      loc:{start:{line:1,column:24},end:{line:1,column:34},source:''},
                      left: {
                        type: 'PrivateIdentifier',
                        name: 'x',
                        loc:{start:{line:1,column:24},end:{line:1,column:26},source:''}
                      },
                      operator: 'in',
                      right: {
                        type: 'ThisExpression',
                        loc:{start:{line:1,column:30},end:{line:1,column:34},source:''}
                      }
                    }
                  }
                ]
              }
            }
          }
        ]
      }
    }
  ]
}

tokens (20x):
       ID_class IDENT PUNC_CURLY_OPEN ID_PRIVATE_IDENT PUNC_SEMI
       ID_static PUNC_CURLY_OPEN PUNC_CURLY_CLOSE PUNC_STAR IDENT
       PUNC_PAREN_OPEN PUNC_PAREN_CLOSE PUNC_CURLY_OPEN
       ID_PRIVATE_IDENT ID_in ID_this ASI PUNC_CURLY_CLOSE
       PUNC_CURLY_CLOSE
`````

### Strict mode

Parsed with script goal but as if it was starting with `"use strict"` at the top.

_Output same as sloppy mode._

### Module goal

Parsed with the module goal.

_Output same as sloppy mode._

### Sloppy mode with AnnexB

Parsed with script goal with AnnexB rules enabled and as if the code did not start with strict mode header.

_Output same as sloppy mode._

### Module goal with AnnexB

Parsed with the module goal with AnnexB rules enabled.

_Output same as sloppy mode._

## AST Printer

Printer output different from input [sloppy][annexb:no]:

````js
class C{
#x;
static {}
* m(){#x in this;};
}
````

Produces same AST
